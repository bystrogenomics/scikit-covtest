...
def derivative(func, x, dx=1e-6, order=3, *args, **kwargs):
    ...
    - x: scalar float
    - dx: step size
    - order: must be odd (3, 5, 7, 9)
    """
    if order == 3:
        weights = np.array([-1, 0, 1]) / (2 * dx)
        points = x + dx * np.array([-1, 0, 1])
        return np.dot(weights, [func(pt, *args, **kwargs) for pt in points])
    elif order == 5:
        weights = np.array([1, -8, 0, 8, -1]) / (12 * dx)
        points = x + dx * np.array([-2, -1, 0, 1, 2])
        return np.dot(weights, [func(pt, *args, **kwargs) for pt in points])
    elif order == 7:
        weights = np.array([-1, 9, -45, 0, 45, -9, 1]) / (60 * dx)
        ...
        raise ValueError("Only orders 3, 5, 7, or 9 are supported.")
